...
// RequestContext returns the APIGatewayProxyRequest value stored in ctx.
func RequestContext(ctx context.Context) (events.APIGatewayProxyRequest, bool) {
	c, ok := ctx.Value(requestContextKey).(events.APIGatewayProxyRequest)
	return c, ok
}
